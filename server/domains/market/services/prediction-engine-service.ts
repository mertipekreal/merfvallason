/**
 * Prediction Engine Service v2.0
 * Six-layer market prediction system with 65-70% accuracy target
 * 
 * Combines:
 * 1. Hard Data Layer (30%) - OHLCV, Options, Dark Pool, Institutional
 * 2. Technical Layer (25%) - ICT Smart Money Concepts
 * 3. SAM Layer (25%) - Subconscious Analysis Model
 * 4. Economic Layer (10%) - FRED indicators
 * 5. Emotion Layer (5%) - Dijital Duygu Pusulasi (FinBERT sentiment)
 * 6. Microstructure Layer (5%) - NASDAQ Sinyal Avcisi (signal scoring)
 */
...
import { emotionCompassService } from './emotion-compass-service';
...
export interface PredictionResult {
  ...
  layerBreakdown: {
    ...
    emotionScore: number;      // New: Dijital Duygu Pusulasi
    microstructureScore: number; // New: NASDAQ Sinyal Avcisi
    weights: {
      hardData: number;
      technical: number;
      sam: number;
      economic: number;
      emotion: number;         // New: 5%
      microstructure: number;  // New: 5%
    };
  };
  keyFactors: {
...
const DEFAULT_WEIGHTS = {
  ...
  economic: 0.10,       // 10% - FRED indicators, market regime (reduced from 20%)
  emotion: 0.05,        // 5% - Dijital Duygu Pusulasi (FinBERT sentiment)
  microstructure: 0.05  // 5% - NASDAQ Sinyal Avcisi (signal scoring)
};

const MODEL_VERSION = 'v2.0.0-six-layer';
...
/**
 * Calculate prediction confidence based on signal agreement (6-layer model)
 */
function calculateConfidence(layerScores: { 
  ...
  // Include all 6 layers in confidence calculation
  const scores = [
    layerScores.hardDataScore,
    layerScores.technicalScore,
    layerScores.samScore,
    layerScores.economicScore,
    layerScores.emotionScore ?? 0,
    layerScores.microstructureScore ?? 0
  ];
  ...
  // Calculate average magnitude across all 6 layers
  const avgMagnitude = scores.reduce((sum, s) => sum + Math.abs(s), 0) / 6;
  ...
  if (signAgreement) confidence += 25;
    ...
    // Check how many agree (6 layers now)
    const positiveCount = scores.filter(s => s > 0).length;
    const majorityAgree = positiveCount >= 4 || positiveCount <= 2;
...
export async function generatePrediction(
  ...
  console.log(`[PredictionEngine] Generating ${horizonDays}-day prediction for ${symbol} (v2.0 - 6 layers)`);

  // Generate features from traditional 4 layers
  ...
  // Calculate traditional layer scores
  const layerScores = calculateLayerScores(features);

  // === NEW: Get Emotion Layer Score (Dijital Duygu Pusulasi) ===
  let emotionScore = 0;
  try {
    const emotionData = await emotionCompassService.getEmotionScore(symbol);
    if (emotionData && emotionData.score !== 0) {
      // Score is already -100 to 100, normalize to -1 to 1
      emotionScore = emotionData.score / 100;
    }
  } catch (error: any) {
    console.log(`[PredictionEngine] Emotion layer unavailable: ${error.message}`);
  }

  // === NEW: Get Microstructure Layer Score (NASDAQ Sinyal Avcisi) ===
  let microstructureScore = 0;
  try {
    const signalData = await signalScorerService.getMicrostructureScore(symbol);
    if (signalData && signalData.score !== 0) {
      // Score is composite, normalize to -1 to 1
      microstructureScore = Math.max(-1, Math.min(1, signalData.score / 100));
    }
  } catch (error: any) {
    console.log(`[PredictionEngine] Microstructure layer unavailable: ${error.message}`);
  }
  ...
  // Ensure weights have all 6 layers (add defaults if missing from legacy 4-layer engine)
  const fullWeights = {
    hardData: optimizedWeights.hardData || DEFAULT_WEIGHTS.hardData,
    technical: optimizedWeights.technical || DEFAULT_WEIGHTS.technical,
    sam: optimizedWeights.sam || DEFAULT_WEIGHTS.sam,
    economic: optimizedWeights.economic || DEFAULT_WEIGHTS.economic,
    emotion: (optimizedWeights as any).emotion || DEFAULT_WEIGHTS.emotion,
    microstructure: (optimizedWeights as any).microstructure || DEFAULT_WEIGHTS.microstructure,
  };
  
  // Check if current conditions match high-success patterns (all 6 layers)
  ...
  // Calculate weighted composite score with ALL 6 LAYERS
  const compositeScore = 
    layerScores.hardDataScore * fullWeights.hardData +
    layerScores.technicalScore * fullWeights.technical +
    layerScores.samScore * fullWeights.sam +
    layerScores.economicScore * fullWeights.economic +
    emotionScore * fullWeights.emotion +
    microstructureScore * fullWeights.microstructure;
  ...
  // Calculate confidence with all 6 layers
  ...
  const result: PredictionResult = {
    ...
    layerBreakdown: {
      ...
      emotionScore,         // New: Dijital Duygu Pusulasi
      microstructureScore,  // New: NASDAQ Sinyal Avcisi
      weights: fullWeights  // Use LEARNED weights with all 6 layers
    },
    keyFactors,
    featureSnapshotId,
